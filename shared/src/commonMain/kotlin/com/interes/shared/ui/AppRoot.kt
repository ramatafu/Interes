 ...
 * SideToolbar.kt), справа — RightToolbar (см. RightToolbar.kt), оба видны
 * поверх любого из этих экранов; раздела "Настройки" в приложении больше
 * нет — то немногое, что в нём было (резервное копирование, информация о
 * программе), переехало в сам SideToolbar.
 *
 ...
 *
 * SideToolbar, RightToolbar и элементы управления просмотрщика специально
 * вынесены СНАРУЖИ этого затухающего слоя — панели инструментов должны
 * оставаться видимыми и кликабельными, даже когда окно почти полностью
 * прозрачно.
 */
@Composable
fun InteresRoot(
    ...
    InteresTheme {
        Box(modifier = Modifier.fillMaxSize()) {
            ...
            // же визуальный результат (панели слева и справа фиксированной
            // ширины, контент занимает оставшееся) совершенно другим,
            // гарантированно рабочим в этом проекте механизмом (Box-выравнивание
            // уже используется, например, в BoardCard).
            //
            // padding(start = ToolbarWidth, end = RightToolbarWidth) — сдвиг
            // контента под ОБА боковых тулбара, чтобы ни левая, ни правая панель
            // не перекрывали контент.
            Box(
                modifier = Modifier
                    .fillMaxSize()
            ...
            // Левая панель — поверх содержимого (которое сдвинуто под неё
            ...
            // Правая панель — поверх содержимого (которое сдвинуто под неё
            // отступом padding(end = RightToolbarWidth) выше), прижата к
            // правому краю на всю высоту окна.
            RightToolbar(
